...
import { LayoutDashboard, Users, Eye, Building2, TrendingUp, ShieldCheck } from "lucide-react";
import { LoginView } from "@/features/auth/components/LoginView";
import { ConceptHeatmap } from "@/features/parent/components/ConceptHeatmap";

export default function HomePage() {
  const { activeAgent, masteryLevel, student, fetchStudent, fetchAgents } = useAgentStore();
  const [view, setView] = useState<"council" | "dashboard" | "parent" | "partner">("council");
    ...
    <main className="min-h-screen bg-white text-navy font-sans">
      ...
      <header className="border-b border-navy/5 px-8 py-4 flex justify-between items-center sticky top-0 bg-white/80 backdrop-blur-md z-50">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 bg-emerald rounded-lg flex items-center justify-center text-white font-bold italic">G</div>
          <span className="text-xl font-bold tracking-tight">GenEd Portal</span>
        </div>
        <nav className="flex gap-8 text-sm font-medium text-navy/60">
          ...
          {userRole === "partner" && (
            <button 
              ...
              className={`flex items-center gap-2 transition-colors ${view === "partner" ? "text-navy" : "hover:text-navy"}`}
            >
        ...
        <div className="flex gap-4">
          <div className="w-8 h-8 bg-academic-grey rounded-full border border-navy/10 flex items-center justify-center text-[10px] font-bold">A</div>
        </div>
      </header>
          ...
          {view === "dashboard" ? (
            ...
            >
              <header className="space-y-2">
                <h2 className="text-4xl font-bold tracking-tight">Parent Knowledge Insights</h2>
                <p className="text-navy/40">Observing the conceptual journey of {student?.name || 'Scholar'}.</p>
              </header>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="p-8 bg-academic-grey border border-navy/5 rounded-3xl space-y-4">
                      <div className="flex justify-between items-center">
                        <h4 className="font-bold text-navy uppercase text-[10px] tracking-widest leading-none">Cognitive Effort</h4>
                        <TrendingUp size={16} className="text-emerald" />
                      </div>
                      <p className="text-4xl font-bold text-navy">92%</p>
                      <p className="text-xs text-navy/40 leading-relaxed italic">"Arjun is showing high resilience in {activeAgent?.name || 'Mathematics'}."</p>
                    </div>
                    <div className="p-8 bg-navy text-white rounded-3xl space-y-4 shadow-xl shadow-navy/20">
                      ...
                      <button className="text-[10px] font-bold underline decoration-emerald decoration-2 text-emerald">View Achievement Logs</button>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-8">
                  ...
                  <div className="p-8 bg-white border border-navy/5 rounded-3xl shadow-sm text-center space-y-6">
                    <div className="space-y-2">
                      <p className="text-xs font-bold text-navy/40 uppercase tracking-[0.2em]">Parent Council Call</p>
                      <p className="text-lg font-bold">Friday, 4 PM</p>
                    </div>
                    <button className="w-full py-4 bg-navy text-white text-xs font-bold rounded-2xl hover:bg-emerald transition-all shadow-lg shadow-navy/10">Schedule Sync</button>
                  </div>
                </div>
              </div>
            </motion.div>
            ...
            >
              <div className="space-y-2">
                <h1 className="text-4xl font-bold text-navy">District Ecosystem Oversight</h1>
                <p className="text-navy/40 font-sans">Multi-student mastery tracking and curriculum ingestion.</p>
              </div>
              <PartnerAdmin />
            </motion.div>
          ) : (
