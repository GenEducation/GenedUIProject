/**
 * useMomentsStore — wake-up alarms state, ported from the web parent store.
 * Optimistic local list updates around the momentsService CRUD calls.
 */
...
export const useMomentsStore = create<MomentsState>((set) => ({
  ...
  loadMoments: async (userId) => {
    ...
    try {
      ...
      set({ moments: Array.isArray(moments) ? moments : [] });
    } catch (e) {
