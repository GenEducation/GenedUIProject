/**
 * Lab service — mobile port of the web app's src/features/lab/services/labService.ts.
 * All calls go through authFetch which injects the Bearer token and handles 401/403.
 */
...
export const labService = {
  ...
  createLab: async (payload: CreateLabRequest): Promise<LabResponse> => {
    const res = await authFetch(`${BASE}/lab/labs`, {
      method: "POST",
      headers: jsonHeaders,
  ...
  listLabs: async (partnerId: string): Promise<LabResponse[]> => {
    const res = await authFetch(`${BASE}/lab/labs?partner_id=${encodeURIComponent(partnerId)}`);
    return res.json();
  },

  updateLab: async (labId: string, payload: LabUpdateRequest): Promise<LabResponse> => {
    const res = await authFetch(`${BASE}/lab/labs/${encodeURIComponent(labId)}`, {
      method: "PATCH",
      headers: jsonHeaders,
      body: JSON.stringify(payload),
    });
  ...
  registerDevice: async (payload: RegisterDeviceRequest): Promise<DeviceProvisionResponse> => {
    const res = await authFetch(`${BASE}/lab/devices`, {
      method: "POST",
      headers: jsonHeaders,
      body: JSON.stringify(payload),
    });
  ...
  rotateToken: async (deviceId: string): Promise<DeviceProvisionResponse> => {
    const res = await authFetch(`${BASE}/lab/devices/${encodeURIComponent(deviceId)}/rotate-token`, {
      method: "POST",
    });
    return res.json();
  },
  ...
  listDevices: async (labId: string): Promise<DeviceResponse[]> => {
    const res = await authFetch(`${BASE}/lab/labs/${encodeURIComponent(labId)}/devices`);
    return res.json();
  },

  updateDevice: async (deviceId: string, payload: DeviceUpdateRequest): Promise<DeviceResponse> => {
    const res = await authFetch(`${BASE}/lab/devices/${encodeURIComponent(deviceId)}`, {
      method: "PATCH",
      headers: jsonHeaders,
      body: JSON.stringify(payload),
    });
  ...
  revokeDevice: async (deviceId: string): Promise<DeviceResponse> => {
    const res = await authFetch(`${BASE}/lab/devices/${encodeURIComponent(deviceId)}/revoke`, {
      method: "POST",
    });
    return res.json();
  },
  ...
  getCatalog: async (partnerId: string, grade?: number): Promise<CatalogResponse> => {
    ...
    const res = await authFetch(`${BASE}/lab/catalog?${qs.toString()}`);
    return res.json();
  },

  // -- Slots ------------------------------------------------------------------
  createSlot: async (payload: CreateSlotRequest): Promise<SlotResponse> => {
    const res = await authFetch(`${BASE}/lab/slots`, {
      method: "POST",
      headers: jsonHeaders,
      body: JSON.stringify(payload),
    });
  ...
  listSlots: async (params?: { labId?: string; onDate?: string }): Promise<SlotResponse[]> => {
    ...
    const res = await authFetch(`${BASE}/lab/slots${suffix}`);
    return res.json();
  },

  updateSlot: async (slotId: string, payload: SlotUpdateRequest): Promise<SlotResponse> => {
    const res = await authFetch(`${BASE}/lab/slots/${encodeURIComponent(slotId)}`, {
      method: "PATCH",
      headers: jsonHeaders,
      body: JSON.stringify(payload),
    });
  ...
  cancelSlot: async (slotId: string): Promise<SlotResponse> => {
    const res = await authFetch(`${BASE}/lab/slots/${encodeURIComponent(slotId)}/cancel`, {
      method: "POST",
    });
    return res.json();
  },
  ...
  activateSlot: async (slotId: string): Promise<ActivateResponse> => {
    const res = await authFetch(`${BASE}/lab/slots/${encodeURIComponent(slotId)}/activate`, {
      method: "POST",
    });
    return res.json();
  },
  ...
  endSlot: async (slotId: string): Promise<SlotResponse> => {
    const res = await authFetch(`${BASE}/lab/slots/${encodeURIComponent(slotId)}/end`, {
      method: "POST",
    });
    return res.json();
  },
  ...
  getBoard: async (slotId: string): Promise<BoardResponse> => {
    const res = await authFetch(`${BASE}/lab/slots/${encodeURIComponent(slotId)}/board`);
    return res.json();
  },

  // -- Allocation actions ---------------------------------------------------------
  bind: async (slotId: string, studentId: string, deviceId: string): Promise<SessionResponse> => {
    const res = await authFetch(`${BASE}/lab/slots/${encodeURIComponent(slotId)}/bind`, {
      method: "POST",
      headers: jsonHeaders,
  ...
  reassign: async (slotId: string, sessionId: string, deviceId: string): Promise<SessionResponse> => {
    const res = await authFetch(
      `${BASE}/lab/slots/${encodeURIComponent(slotId)}/sessions/${encodeURIComponent(sessionId)}/reassign`,
      {
        method: "POST",
        headers: jsonHeaders,
  ...
  resumeIncomplete: async (slotId: string, sessionId: string, deviceId: string): Promise<SessionResponse> => {
    const res = await authFetch(
      `${BASE}/lab/slots/${encodeURIComponent(slotId)}/sessions/${encodeURIComponent(sessionId)}/resume`,
      {
        method: "POST",
        headers: jsonHeaders,
        body: JSON.stringify({ device_id: deviceId }),
  ...
  swap: async (slotId: string, sessionIdA: string, sessionIdB: string): Promise<SessionResponse[]> => {
    const res = await authFetch(`${BASE}/lab/slots/${encodeURIComponent(slotId)}/swap`, {
      method: "POST",
      headers: jsonHeaders,
  ...
  confirmOverride: async (slotId: string, sessionId: string): Promise<SessionResponse> => {
    const res = await authFetch(
      `${BASE}/lab/slots/${encodeURIComponent(slotId)}/sessions/${encodeURIComponent(sessionId)}/confirm-override`,
      { method: "POST" },
    );
    return res.json();
  },
  ...
  markAbsent: async (slotId: string, sessionId: string): Promise<SessionResponse> => {
    const res = await authFetch(
      `${BASE}/lab/slots/${encodeURIComponent(slotId)}/sessions/${encodeURIComponent(sessionId)}/absent`,
      { method: "POST" },
    );
    return res.json();
  },
  ...
  markPresent: async (slotId: string, sessionId: string): Promise<SessionResponse> => {
    const res = await authFetch(
      `${BASE}/lab/slots/${encodeURIComponent(slotId)}/sessions/${encodeURIComponent(sessionId)}/present`,
      { method: "POST" },
    );
    return res.json();
  },
  ...
  endSession: async (slotId: string, sessionId: string): Promise<SessionResponse> => {
    const res = await authFetch(
      `${BASE}/lab/slots/${encodeURIComponent(slotId)}/sessions/${encodeURIComponent(sessionId)}/end`,
      { method: "POST" },
    );
    return res.json();
  },
  ...
  getReport: async (slotId: string): Promise<ClassReportResponse> => {
    const res = await authFetch(`${BASE}/lab/slots/${encodeURIComponent(slotId)}/report`);
    return res.json();
  },
};
