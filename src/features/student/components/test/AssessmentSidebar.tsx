...
import { CreateChapterTestResponse } from "../../types/test";
    ...
    <aside className="w-[450px] hidden xl:flex flex-col sticky top-24 h-[calc(100vh-120px)] bg-white rounded-[40px] border border-slate-100 overflow-hidden shadow-2xl shadow-slate-200/50">
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute top-0 left-0 w-full h-full opacity-[0.03]" 
             style={{ backgroundImage: 'radial-gradient(#042E5C 1px, transparent 1px)', backgroundSize: '32px 32px' }} />
        {/* HUD Frame Elements */}
      ...
      <div className="relative flex-1 overflow-y-auto custom-scrollbar p-12 space-y-24 pt-20">
        {currentTest.sections.map((section, sIdx) => {
          ...
          const isCompleted = section.questions.every(q => !!answers[q.question_id]);
          const isLocked = sIdx > currentSectionIdx && !isCompleted;
          const isStaggeredRight = sIdx % 2 !== 0;
          
          const answeredInSection = section.questions.filter(q => !!answers[q.question_id]).length;
          const totalInSection = section.questions.length;

          return (
            <div key={sIdx} className="relative flex flex-col items-center">
              {/* Zig-Zag Circuit Path */}
              {sIdx < currentTest.sections.length - 1 && (
                <div className="absolute top-16 left-1/2 -translate-x-1/2 w-48 h-32 z-0 pointer-events-none">
                  <svg width="192" height="128" viewBox="0 0 192 128" fill="none" preserveAspectRatio="none">
                    <path 
                      d={isStaggeredRight 
                    ...
                    />
                    {/* Glowing Pulse Node on path */}
                    <motion.circle
                      r="4"
                      fill="#00F2FF"
                      animate={{ offsetDistance: ["0%", "100%"] }}
                      transition={{ duration: 3, repeat: Infinity, ease: "linear" }}
                      style={{ offsetPath: `path('${isStaggeredRight 
                        ? "M 136 0 L 136 64 L 56 64 L 56 128" 
                        : "M 56 0 L 56 64 L 136 64 L 136 128"}')` }}
                    />
                  </svg>
              ...
              >
                ...
                <div className={`
                  ...
                  ${isCompleted ? 'bg-emerald-50 text-emerald-500 border-2 border-emerald-500 shadow-[0_0_25px_rgba(16,185,129,0.2)]' : 
                    isCurrent ? 'bg-white text-cyan-500 border-2 border-cyan-400 shadow-[0_0_30px_rgba(0,242,255,0.3)]' : 
                ...
                `}>
                  ...
                  {/* Status Overlay */}
                  <div className={`absolute -right-1 -bottom-1 w-6 h-6 rounded-full border-2 border-white flex items-center justify-center ${isCompleted ? 'bg-emerald-500' : isLocked ? 'bg-slate-300' : 'bg-cyan-400 shadow-[0_0_10px_rgba(0,242,255,0.5)]'}`}>
                    {isCompleted ? <Check size={12} className="text-white" strokeWidth={4} /> : isLocked ? <Lock size={12} className="text-white" /> : <div className="w-1.5 h-1.5 bg-white rounded-full animate-ping" />}
                  </div>
                </div>

                {/* Info Panel - Absolute Position relative to Node */}
                <AnimatePresence>
                  {(isCurrent || hoveredIdx === sIdx) && (
                    <motion.div 
                      initial={{ opacity: 0, x: isStaggeredRight ? -40 : 40, scale: 0.9 }}
                      animate={{ opacity: 1, x: isStaggeredRight ? -240 : 80, scale: 1 }}
                      exit={{ opacity: 0, x: isStaggeredRight ? -40 : 40, scale: 0.9 }}
                      className={`
                        absolute top-1/2 -translate-y-1/2 w-56 p-5 rounded-2xl border backdrop-blur-xl transition-all z-30 pointer-events-none
                        ${isCurrent ? 'border-cyan-200 bg-white shadow-2xl shadow-cyan-100/50' : 'border-slate-100 bg-white/90 shadow-xl shadow-slate-100/50'}
                      `}
                      style={{ clipPath: 'polygon(0 0, 90% 0, 100% 15%, 100% 100%, 10% 100%, 0 85%)' }}
                    >
                      <div className="space-y-2">
                        <div className="flex items-center justify-between">
                          <span className={`text-[9px] font-black uppercase tracking-[0.2em] ${isCurrent ? 'text-cyan-500' : 'text-slate-400'}`}>
                            {section.tier}
                          </span>
                          <Sparkles size={10} className={isCurrent ? 'text-cyan-400 animate-pulse' : 'text-slate-200'} />
                        </div>
                        <h4 className={`text-[13px] font-black uppercase truncate ${isCurrent ? 'text-slate-800' : 'text-slate-400'}`}>
                          {section.main_heading}
                        </h4>
                        <div className="pt-3 flex flex-col gap-2">
                          <div className="flex items-center justify-between">
                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                              {answeredInSection}/{totalInSection} Completed
                            </span>
                          </div>
                          <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden flex gap-1 p-[1px]">
                            {section.questions.map((q, qIdx) => (
                              <div 
                                key={qIdx}
                                className={`flex-1 rounded-full transition-all duration-500 ${!!answers[q.question_id] ? 'bg-cyan-400 shadow-[0_0_5px_rgba(0,242,255,0.5)]' : 'bg-slate-200'}`}
                              />
                            ))}
                          </div>
                        </div>
