...
interface SubjectOnboardingCelebrationProps {
  subject: string;
  onDismiss: () => void;
}
...
export function SubjectOnboardingCelebration({ subject, onDismiss }: SubjectOnboardingCelebrationProps) {
  ...
  return (
    <div className="absolute inset-0 z-[100] flex items-center justify-center p-4 bg-[#042e5c]/60 backdrop-blur-sm rounded-2xl">
      {/* Confetti */}
      {showConfetti && (
      ...
      >
        ...
        <div className="relative bg-gradient-to-br from-[#042E5C] to-[#0a4a8a] px-8 pt-10 pb-14 text-center overflow-hidden">
          ...
          >
            Your {subject} learning journey begins now
          </motion.p>
        </div>
        ...
        <div className="px-8 pb-8 -mt-2 space-y-5">
          ...
          >
            ...
            <div className="space-y-0.5">
              <p className="text-xs font-black text-[#042e5c] uppercase tracking-wider">Agent Ready</p>
              <p className="text-sm text-[#042e5c]/60 leading-snug">
                Start chatting to explore chapters and build your mastery in {subject}!
              </p>
            </div>
          </motion.div>
